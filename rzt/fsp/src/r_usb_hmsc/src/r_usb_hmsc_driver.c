...
BSP_ALIGN_VARIABLE(32) usb_msc_cbw_t      g_usb_hmsc_cbw[USB_NUM_USBIP][USB_MAXSTRAGE] USB_BUFFER_PLACE_IN_SECTION;         /* CBW headder */
BSP_ALIGN_VARIABLE(32) usb_msc_csw_t      g_usb_hmsc_csw[USB_NUM_USBIP][USB_MAXSTRAGE] USB_BUFFER_PLACE_IN_SECTION;         /* CSW headder */
...
uint8_t * g_p_usb_hmsc_device_table[USB_NUM_USBIP] USB_BUFFER_PLACE_IN_SECTION;
uint8_t * g_p_usb_hmsc_config_table[USB_NUM_USBIP] USB_BUFFER_PLACE_IN_SECTION;
uint8_t * g_p_usb_hmsc_interface_table[USB_NUM_USBIP];
uint16_t  g_usb_hmsc_devaddr[USB_NUM_USBIP];

usb_utr_t g_usb_hmsc_class_control[USB_NUM_USBIP] USB_BUFFER_PLACE_IN_SECTION;
uint8_t   g_usb_hmsc_class_data[USB_NUM_USBIP][USB_HMSC_CLSDATASIZE] USB_BUFFER_PLACE_IN_SECTION;
...
static usb_ctrl_trans_t string_setup[USB_NUM_USBIP] USB_BUFFER_PLACE_IN_SECTION;
static usb_ctrl_trans_t get_max_lun_table USB_BUFFER_PLACE_IN_SECTION;
...
#else
    R_USB_HstdChangeDeviceState(ptr, (usb_cb_t) &usb_hstd_dummy_function, toggle, pipe);
#endif
...
#else
    err   = R_USB_HstdChangeDeviceState(ptr, complete, USB_DO_CLR_STALL, pipe);
#endif
 #endif                                /* (BSP_CFG_RTOS == 2) */
                ...
                if (USB_EP_IN == (pdescriptor[2] & USB_EP_DIRMASK))
                {
                    g_usb_hmsc_in_pipe[ip][side] = usb_hstd_make_pipe_reg_info(ip,
                                                                               ...
                                                                               &g_usb_hmsc_pipe_table[side][
                                                                                   USB_PIPE_DIR_IN]);
                    epnum = R_USB_HMSC_ConvertEndpointNum(g_usb_hmsc_in_pipe[ip][side], USB_PIPE_DIR_IN);
                    g_usb_hmsc_in_pipe[ip][side]  = (uint8_t) R_USB_HstdGetPipeID(g_usb_hmsc_in_pipe[ip][side], epnum);
                }
                else
                {
                    g_usb_hmsc_out_pipe[ip][side] = usb_hstd_make_pipe_reg_info(ip,
                                                                                ...
                                                                                &g_usb_hmsc_pipe_table[side][
                                                                                    USB_PIPE_DIR_OUT]);
                    epnum = R_USB_HMSC_ConvertEndpointNum(g_usb_hmsc_out_pipe[ip][side], USB_PIPE_DIR_OUT);
                    g_usb_hmsc_out_pipe[ip][side]  = (uint8_t) R_USB_HstdGetPipeID(g_usb_hmsc_out_pipe[ip][side], epnum);
                }
...
static void usb_hmsc_detach (usb_utr_t * ptr, uint16_t addr, uint16_t data2)
{
    ...
    if (USB_ERROR != side)
    {
        ...
        usb_hmsc_clr_pipe_table (side, USB_PIPE_DIR_IN);
        usb_hmsc_clr_pipe_table (side, USB_PIPE_DIR_OUT);
...
uint8_t usb_hmsc_convert_epnum (uint16_t pipe_id, uint8_t dir)
{
    if(1 >= pipe_id)
    {
        return (uint8_t) (g_usb_hmsc_pipe_table[pipe_id - 1][dir].pipe_cfg & 0x000F);
    }
...
/*************************************************************************//**
* Function Name   : R_USB_HMSC_ConvertEndpointNum
...
uint8_t R_USB_HMSC_ConvertEndpointNum (uint16_t pipe, uint8_t dir)
{
   return usb_hmsc_convert_epnum(pipe, dir);
...
void usb_hmsc_clr_pipe_table (uint16_t side, uint16_t dir)
{
    ...
    {
        /* Check use block */
        if (USB_TRUE == g_usb_hmsc_pipe_table[pipe_id][dir].use_flag)
        {
            /* Check USB Device address */
            if ((device_address << USB_DEVADDRBIT) ==
                (uint16_t) (g_usb_hmsc_pipe_table[pipe_id][dir].pipe_maxp & USB_DEVSEL))
            {
#endif
                /* Clear use block */
                g_usb_hmsc_pipe_table[side][dir].pipe_buf  = USB_FALSE;
                g_usb_hmsc_pipe_table[side][dir].pipe_cfg  = USB_NULL;
                g_usb_hmsc_pipe_table[side][dir].pipe_maxp = USB_NULL;
                g_usb_hmsc_pipe_table[side][dir].pipe_peri = USB_NULL;
