...
#if IIC_MASTER_CFG_DMAC_ENABLE
void iic_master_txi_dmac_isr(IRQn_Type irq);
void iic_master_rxi_dmac_isr(IRQn_Type irq);
...
static void iic_master_abort_seq_master (iic_master_instance_ctrl_t * const p_ctrl, bool iic_reset)
{
    ...
    IIC_MASTER_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICIER, IIC_MASTER_INTERRUPT_ENABLE_INIT_MASK, timeout_count);
}
...
static void iic_master_open_hw_master (iic_master_instance_ctrl_t * const p_ctrl, i2c_master_cfg_t const * const p_cfg)
{
     ...
     */
    p_ctrl->p_reg->ICFER = IIC_MASTER_FUNCTION_ENABLE_INIT_SETTINGS;
...
static fsp_err_t iic_master_run_hw_master (iic_master_instance_ctrl_t * const p_ctrl)
{
    ...
    uint32_t timeout_count = ((1U << (4U - cpufsel)) * p_ctrl->p_reg->ICBRL) >> 2U;

    /* Bus free time is counted by the Internal reference clock, so consider the division ratio. */
    timeout_count <<= p_ctrl->p_reg->ICMR1_b.CKS;
...
static void iic_master_rxi_master (iic_master_instance_ctrl_t * p_ctrl)
{
    ...
    if (false == p_ctrl->dummy_read_completed)
    {
        ...
        if ((NULL != p_ctrl->p_cfg->p_transfer_rx) && (p_ctrl->read) && (p_ctrl->total > 3U))
        {
            ...
            p_ctrl->p_cfg->p_transfer_rx->p_cfg->p_info->length = p_ctrl->total - 3U;
...
static void iic_master_tei_master (iic_master_instance_ctrl_t * p_ctrl)
{
    ...
    else if (0U == p_ctrl->remain)
    {
        ...
        else
        {
             ...
             * See 'Note' under Table "Interrupt sources" of the RZ microprocessor manual. */
            timeout_count = IIC_MASTER_PERIPHERAL_REG_MAX_WAIT;
            IIC_MASTER_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICSR2_b.STOP, 0U, timeout_count);
...
static void iic_master_err_master (iic_master_instance_ctrl_t * p_ctrl)
{
     ...
     * See 'Note' under Table "Interrupt sources" of the RZ microprocessor manual */
    uint32_t timeout_count = IIC_MASTER_PERIPHERAL_REG_MAX_WAIT;
    IIC_MASTER_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICSR2 & IIC_MASTER_STATUS_REGISTER_2_ERR_MASK, 0U, timeout_count);
             ...
             ((p_ctrl->restarted) && (errs_events & (uint8_t) IIC_MASTER_ERR_EVENT_START)))
    {
        ...
        if (false == p_ctrl->err)      /* Successful transaction */
        {
             ...
             * See 'Note' under Table "Interrupt sources" of the RZ microprocessor manual */
            timeout_count = IIC_MASTER_PERIPHERAL_REG_MAX_WAIT;
            IIC_MASTER_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICIER, IIC_MASTER_INTERRUPT_ENABLE_INIT_MASK,
                                              timeout_count);
        }
        else if ((errs_events & (uint8_t) IIC_MASTER_ERR_EVENT_STOP))
        {
    ...
    else
    {
        ...
        if (1U == p_ctrl->addr_remain)
        {
            if ((NULL != p_ctrl->p_cfg->p_transfer_tx) && !(p_ctrl->read) && (p_ctrl->total > 0U))
            {
                ...
                p_ctrl->p_cfg->p_transfer_tx->p_cfg->p_info->length = p_ctrl->remain;
                                                ...
                                                iic_master_transfer_dir_t    direction)
{
    ...
    if (IIC_MASTER_TRANSFER_DIR_READ == direction)
    {
        ...
        p_info->transfer_settings_word = IIC_MASTER_DMAC_RX_TRANSFER_SETTINGS;
    }
    else
    {
        irq = p_ctrl->p_cfg->txi_irq;
        p_info->transfer_settings_word = IIC_MASTER_DMAC_TX_TRANSFER_SETTINGS;
    }
...
/*******************************************************************************************************************//**
 * Receive data full interrupt routine called when the DMA transfer is completed.
 *
 * This function implements the IIC Receive buffer full ISR routine.
 *
 **********************************************************************************************************************/
