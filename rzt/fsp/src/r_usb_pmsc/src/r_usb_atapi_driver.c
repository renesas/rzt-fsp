...
#define USB_VAL_0X70                       (0x70)
#define USB_VAL_0X24                       (0x24)
#define USB_VAL_0X1B                       (0x1B)
#define USB_VAL_0X0A                       (0x0A)
...
void        pmsc_atapi_command_init(void);
...
static uint8_t g_usb_pmsc_media_buffer[USB_ATAPI_BLOCK_UNIT * USB_CFG_PMSC_TRANS_COUNT +
                                       16] USB_BUFFER_PLACE_IN_SECTION;
static usb_pmsc_cdb_t * g_usb_atapi_cbwcb USB_BUFFER_PLACE_IN_SECTION; /* CBWCB pointer */
static uint32_t g_usb_atapi_cur_lba       USB_BUFFER_PLACE_IN_SECTION; /* the current Logical Block Address */
...
void pmsc_atapi_init (void)
{
    ...
    pmsc_atapi_command_init();
}                                      /* End of function pmsc_atapi_init() */

/***********************************************************************************************************************
 * Function Name: pmsc_atapi_command_init
 * Description  :
 * Arguments    : none
 * Return value : none
 ***********************************************************************************************************************/
void pmsc_atapi_command_init (void)
{
    ...
    /******* INQUIRY TABLE *******/
    for (i = 0; i < USB_ATAPI_INQUIRY_SIZE; i++)
    {
        g_usb_atapi_inquiry_tbl[i] = 0x00;
    }

    g_usb_atapi_inquiry_tbl[1] = USB_VALUE_RMB;                /* [1]b7:RMB(ARemovable Medium Bit), b6-b0:Reserved */
    g_usb_atapi_inquiry_tbl[3] = 0x01;                         /* [3]b7-b4:Reserved, b3-b0:Response Data Format */
    g_usb_atapi_inquiry_tbl[4] = USB_VALUE_ADDITIONAL_LEN_20H; /* [4]Additional Length(Number of bytes follwing this one) */

    /******* READ FORMAT CAPACITY TABLE *******/
    for (i = 0; i < USB_ATAPI_READ_FORMAT_CAPACITY_SIZE; i++)
    {
        g_usb_atapi_read_f_capa_tbl[i] = 0x00;
    }

    g_usb_atapi_read_f_capa_tbl[3]  = 0x10; /* Capacity List Header Length */
    g_usb_atapi_read_f_capa_tbl[8]  = 0x02; /* Current/Maximum Capacity Descriptor */
    g_usb_atapi_read_f_capa_tbl[10] = 0x02; /* Current/Maximum Capacity Descriptor */
    g_usb_atapi_read_f_capa_tbl[18] = 0x02; /* Formattable Capacity Descriptor */

    /******* READ CAPACITY TABLE *******/
    for (i = 0; i < USB_ATAPI_READ_CAPACITY_SIZE; i++)
    {
        g_usb_atapi_read_capa_tbl[i] = 0x00;
    }

    /******* REQUEST SENSE TABLE *******/
    for (i = 0; i < USB_ATAPI_REQUEST_SENSE_SIZE; i++)
    {
        g_usb_atapi_rs_tbl[i] = 0x00;
    }

    g_usb_atapi_rs_tbl[0]  = USB_VAL_0X70; /* [0]b7:Valid, b6-b0:Error Code(70h) */
    g_usb_atapi_rs_tbl[2]  = 0x05;         /* [2]b7-b5:Vendor Specific, b4:reserved, B3-b0: Sense Key */
    g_usb_atapi_rs_tbl[7]  = USB_VAL_0X0A; /* [7]Additional Sense Length */
    g_usb_atapi_rs_tbl[12] = USB_VAL_0X24; /* [12]ASC(Additonal Sense Code(Mandatory)) */

    /******* MODE SENSE (MODE PARAMETER) TABLE *******/
    for (i = 0; i < USB_ATAPI_MODE_SENSE10_MODE_PARAM_SIZE; i++)
    {
        g_usb_atapi_ms_mp_tbl[i] = 0x00;
    }

    /******* Mode Sense data (ReadWrite Error Recovery Page 12byte) TABLE *******/
    for (i = 0; i < USB_ATAPI_MODE_SENSE10_ERR_RCVR_P_SIZE; i++)
    {
        g_usb_atapi_ms_rcvr_tbl[i] = 0x00;
    }

    /* Mode Parameter List */
    g_usb_atapi_ms_rcvr_tbl[1] = 0x0C;         /* [1]Mode Data Length */
    /* Page(ReadWrite Error Recovery Page 12byte) */
    g_usb_atapi_ms_rcvr_tbl[8] = 0x01;         /* [0]b7:PS, b6:Reserved, b5-b0:Page Code(01h) */
    g_usb_atapi_ms_rcvr_tbl[9] = USB_VAL_0X0A; /* [1]Page Length(0Ah) */

    /******* Mode Sense data (Cashing Page 12byte) *******/
    for (i = 0; i < USB_ATAPI_MODE_SENSE10_CASHING_P_SIZE; i++)
    {
        g_usb_atapi_ms_cash_tbl[i] = 0x00;
    }

    /* Mode Parameter List */
    g_usb_atapi_ms_cash_tbl[1] = 0x0C;         /* [1]Mode Data Length */
    /* Page(Cashing Page 12byte) */
    g_usb_atapi_ms_cash_tbl[8] = 0x08;         /* [0]b7:PS, b6:Reserved, b5-b0:Page Code(08h) */
    g_usb_atapi_ms_cash_tbl[9] = USB_VAL_0X0A; /* [1]Page Length(0Ah) */

    /******* Mode Sense data (Removable Block Access Capacities Page 12byte) *******/
    for (i = 0; i < USB_ATAPI_MODE_SENSE10_CAP_P_SIZE; i++)
    {
        g_usb_atapi_ms_capa_tbl[i] = 0x00;
    }

    /* Mode Parameter List */
    g_usb_atapi_ms_capa_tbl[1] = 0x0C;         /* [1]Mode Data Length */
    /* Page(Removable Block Access Capacities Page 12byte) */
    g_usb_atapi_ms_capa_tbl[8] = USB_VAL_0X1B; /* [0]b7:PS, b6:Reserved, b5-b0:Page Code(1Bh) */
    g_usb_atapi_ms_capa_tbl[9] = USB_VAL_0X0A; /* [1]Page Length(0Ah) */

    /******* Mode Sense data (Timer & Protect Page 8byte) *******/
    for (i = 0; i < USB_ATAPI_MODE_SENSE10_OP_CMD_SIZE; i++)
    {
        g_usb_atapi_ms_op_cmd_tbl[i] = 0x00;
    }

    /* Mode Parameter List */
    g_usb_atapi_ms_op_cmd_tbl[1] = 0x08;         /* [1]Mode Data Length */
    /* Page(Removable Block Access Capacities Page 12byte) */
    g_usb_atapi_ms_op_cmd_tbl[8] = 0x1C;         /* [0]b7:PS, b6:Reserved, b5-b0:Page Code(1Bh) */
    g_usb_atapi_ms_op_cmd_tbl[9] = USB_VAL_0X0A; /* [1]Page Length(0Ah) */

    /******* Mode Sense data (All Page) *******/
    for (i = 0; i < USB_ATAPI_MODE_SENSE10_ALL_P_SIZE; i++)
    {
        g_usb_atapi_ms_all_tbl[i] = 0x00;
    }

    /* Mode Parameter List */
    g_usb_atapi_ms_all_tbl[1] = USB_VALUE_ALL_PAGE_LENGTH; /* [1]Mode Data Length */
    /* Page(Removable Block Access Capacities Page 12byte) */
    g_usb_atapi_ms_all_tbl[8] = 0x01;                      /* [0]b7:PS, b6:Reserved, b5-b0:Page Code(1Bh) */
    g_usb_atapi_ms_all_tbl[9] = USB_VAL_0X0A;              /* [1]Page Length(0Ah) */
    /* Page(Cashing Page 12byte) */
    g_usb_atapi_ms_all_tbl[20] = 0x08;                     /* [0]b7:PS, b6:Reserved, b5-b0:Page Code(08h) */
    g_usb_atapi_ms_all_tbl[21] = USB_VAL_0X0A;             /* [1]Page Length(0Ah) */
    /* Page(Cashing Page 12byte) */
    g_usb_atapi_ms_all_tbl[USB_VALUE_32] = USB_VAL_0X1B;   /* [0]b7:PS, b6:Reserved, b5-b0:Page Code(08h) */
    g_usb_atapi_ms_all_tbl[USB_VALUE_33] = USB_VAL_0X0A;   /* [1]Page Length(0Ah) */

    /******* Mode Sense data (Page code error) *******/
    for (i = 0; i < USB_ATAPI_MODE_SENSE10_P_CODE_ERR_SIZE; i++)
    {
        g_usb_atapi_ms_err_tbl[i] = 0x00;
    }
}                                      /* End of function pmsc_atapi_command_init() */
