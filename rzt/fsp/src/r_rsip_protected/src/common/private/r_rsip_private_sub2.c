                                        ...
                                        uint64_t                 message_length)
{
    ...
    {
        rsip_ret =
            p_func->p_enc_update((const uint32_t *) p_handle->wrapped_msg,
                                 r_rsip_byte_to_word_convert(p_handle->wrapped_msg_length));
        if (RSIP_RET_PASS == rsip_ret)
        {
            rsip_ret =
                p_func->p_update((const uint32_t *) p_message, r_rsip_byte_to_word_convert((uint32_t) message_length));
                                   ...
                                   uint64_t message_length)
{
    ...
    /* Call function (cast to match the argument type with the primitive function) */
    rsip_ret_t rsip_ret = p_func->p_enc_update(NULL, 0);
    if (RSIP_RET_PASS == rsip_ret)
    {
        rsip_ret =
            p_func->p_update((const uint32_t *) p_message, r_rsip_byte_to_word_convert((uint32_t) message_length));
...
rsip_ret_t r_rsip_kdf_hmac_resume_final (rsip_kdf_hmac_handle_t * p_handle, uint8_t * p_mac)
{
    ...
    {
        rsip_ret =
            p_func->p_final((const uint32_t *) p_handle->buffer, NULL, (uint32_t *) p_mac,
                            r_rsip_byte_to_word_convert(p_handle->buffered_length), 0);
    }
