 ...
 * @param[in]     key_length    Length of HMAC key to be extracted from MAC. 32 to 64 bytes are available.
                                   ...
                                   rsip_wrapped_key_t * const       p_wrapped_key)
{
    ...
    /* Check length */
    FSP_ERROR_RETURN(RSIP_PRV_BYTE_SIZE_KDF_HMAC_SHA256_MIN <= key_length, FSP_ERR_INVALID_SIZE);
    FSP_ERROR_RETURN(RSIP_PRV_BYTE_SIZE_KDF_HMAC_SHA256_MAX >= key_length, FSP_ERR_INVALID_SIZE);

    uint32_t blocks[1] = {bswap_32big((RSIP_PRV_BYTE_SIZE_KDF_HMAC_SHA256_MIN == key_length) ? 1 : 2)};
 ...
 *
 * @param[in,out] p_ctrl        Pointer to control block.
 * @param[in]     p_wrapped_key Pointer to wrapped key of derived MAC.
                                                ...
                                                uint8_t * const                  p_verify_data)
{
    ...
    /* Check key type */
    FSP_ERROR_RETURN(RSIP_ALG_KDF_HMAC == p_wrapped_key->alg, FSP_ERR_CRYPTO_RSIP_KEY_SET_FAIL);

    /* Check configuration */
    FSP_ERROR_RETURN(gp_func_kdf_tls12_prf_verify_data_compute
                     [p_wrapped_key->subtype][RSIP_TLS12_PRF_LABEL_CLIENT_FINISHED],
                     FSP_ERR_NOT_ENABLED);
#endif
    ...
    /* Call function (cast to match the argument type with the primitive function) */
    rsip_ret_t rsip_ret =
        gp_func_kdf_tls12_prf_verify_data_compute[RSIP_KEY_KDF_HMAC_SHA256][label]((const uint32_t *) p_hash,
                                                                                   (const uint32_t *) p_wrapped_key->
                                                                                   value,
