...
/* PRQA S 2814 ++ # D008 Avoid redundant coding */
/* PRQA S 2880, 2984, 2991, 2995 ++ # D009 Processing for flow verification */
/* PRQA S 3383, 3384 ++ # D013 Never wraps around */
 ...
 *     -# Set CIP_DRV_KEY_CERT_PK_CMP_SRC_ROT to p_cc_key_cert_param->key_cert_pk_cmp_src
 *   -# Else
 *     -# Set CIP_DRV_KEY_CERT_PK_CMP_SRC_IMG_PK to p_cc_key_cert_param->key_cert_pk_cmp_src
 *   -# Get hash algorithm with #cmn_drv_get_hash_algo
 *   -# Set function result to p_cc_key_cert_param->img_pk_hash_algo
 *   -# Cast the p_img_pk_hash_tlv->p_val to uint32_t* type and set it to p_cc_key_cert_param->p_img_pk_hash
 *   -# Set p_img_pk_hash_tlv->byte_len to p_cc_key_cert_param->img_pk_hash_len
 *   -# Get sign algorithm with #cmn_drv_get_sign_algo
 *   -# Set function result to p_cc_key_cert_param->sign_algo
 *   -# Get sign hash algorithm with #cmn_drv_get_sign_hash_algo
 *   -# Set function result to p_cc_key_cert_param->sign_hash_algo
 *   -# Get sign scheme with #cmn_drv_get_sign_scheme
 *   -# Set function result to p_cc_key_cert_param->sign_scheme
 *   -# Cast the p_sign_pk_tlv->p_val to uint32_t* type and set it to p_cc_key_cert_param->p_sign_pk
 *   -# Set p_sign_pk_tlv->byte_len to p_cc_key_cert_param->sign_pk_len
 *   -# Cast the p_sign_tlv->p_val to uint32_t* type and set it to p_cc_key_cert_param->p_sign
 *   -# Set p_sign_tlv->byte_len to p_cc_key_cert_param->sign_len
                                             ...
                                             st_cip_drv_cc_key_cert_param_t * const p_cc_key_cert_param)
{
        ...
        (NULL != p_sign_tlv) && (NULL != p_cc_key_cert_param))
    {
        ...
        p_cc_key_cert_param->sign_scheme      =
            cmn_drv_get_sign_scheme(p_sign_tlv->type, p_cc_key_cert_param->sign_algo);
        ...
        p_cc_key_cert_param->key_cert_sign_len = (uint32_t) (SB_PRV_MANI_HEADER_SIZE + SB_PRV_MANI_TLV_LEN_SIZE +
                                                             p_key_cert_st->tlv_len) -
                                                 (uint32_t) (SB_PRV_TLV_TL_SIZE + p_sign_tlv->byte_len);
                                              ...
                                              st_cip_drv_cc_code_cert_param_t * const p_cc_code_cert_param)
{
        ...
        (NULL != p_sign_tlv) && (NULL != p_cc_code_cert_param))
    {
        ...
        p_cc_code_cert_param->sign_scheme    =
            cmn_drv_get_sign_scheme(p_sign_tlv->type, p_cc_code_cert_param->sign_algo);
 ...
 *   -# Set timing to p_cipher_img_param->timing
 *   -# Set p_code_cert_st->p_header->dest_addr to img_addr
 *   -# Set img_addr to p_cipher_img_param->p_img_src
 *   -# Set p_code_cert_st->p_header->img_len to p_cipher_img_param->img_len
 *   -# Cast the p_img_cip_iv->p_val to uint32_t* type and set it to p_cipher_img_param->p_iv
 *   -# Set p_img_cip_iv->byte_len to p_cipher_img_param->iv_len
 *   -# If (NULL == p_img_cip_info->p_val)
 *     -# Set p_cipher_img_param->p_img_src to p_cipher_img_param->p_img_dst
 *     -# Set CIP_DRV_CIPHER_ALGO_NONE to p_cipher_img_param->cipher_algo
 *     -# Set CIP_DRV_CIPHER_MODE_NONE to p_cipher_img_param->cipher_mode
 *     -# Set CIP_DRV_CIPHER_INFO_KEY_SEL_DEFAULT to p_cipher_img_param->key_select
 *     -# Set CIP_DRV_CIPHER_INFO_IV_SEL_DEFAULT to p_cipher_img_param->iv_select
 *   -# Else
 ...
 *     -# Set function result to p_cipher_img_param->cipher_algo
 *     -# Get Cipher mode with #cmn_drv_get_img_cip_info_mode
 *     -# Set function result to p_cipher_img_param->cipher_mode
 *     -# Set p_img_cip_info_val->key_sel to p_cipher_img_param->key_select
 *     -# Set p_img_cip_info_val->iv_sel to p_cipher_img_param->iv_select
 *     -# If #SB_PRV_TRUE == is_overwrite_image
 *       -# Set p_cipher_img_param->p_img_src to p_cipher_img_param->p_img_dst
 *     -# Else
 *       -# Set p_img_cip_info_val->dest_addr to img_addr
 *       -# Set img_addr to p_cipher_img_param->p_img_dst
 *   -# If (SB_PRV_RET_UNSUPPORTED_ALGO == p_cipher_img_param->cipher_algo) ||
 *         (SB_PRV_RET_UNSUPPORTED_ALGO == p_cipher_img_param->cipher_mode))
 *     -# Set #SB_RET_ERR_MANI_UNSUPPORTED_ALGORITHM to ret
 *   -# Else
 *     -# Do nothing
 * -# Else
 *   -# Set #SB_RET_ERR_INTERNAL_FAIL to ret
 ...
 *   -# cip_ret == CIP_DRV_RET_AUTH_FAIL,             Set #SB_RET_ERR_CRYPTO_AUTH_FAIL to ret
 *   -# cip_ret == CIP_DRV_RET_UNSUPPORTED_ALGORITHM, Set #SB_RET_ERR_CRYPTO_UNSUPPORTED_ALGORITHM to ret
 *   -# cip_ret == CIP_DRV_RET_RESOURCE_CONFLICT,     Set #SB_RET_ERR_CRYPTO_RESOURCE_CONFLICT to ret
 *   -# cip_ret == CIP_DRV_RET_LOWER_IMAGE_VERSION,   Set #SB_RET_ERR_LOWER_IMAGE_VERSION to ret
 *   -# cip_ret == CIP_DRV_RET_PARAM_ERROR,   Set #SB_RET_ERR_CRYPTO_PARAM_ERR to ret
 *   -# default, Set #SB_RET_ERR_CRYPTO_FAIL to ret
 * -# switch cip_ret
 *   -# cip_ret == CIP_DRV_RET_PASS
 *     -# Set (#SB_RET_SUCCESS & #SB_PRV_RET_LOWER_MASK) to ret
 *   -# cip_ret == CIP_DRV_RET_SAME_IMAGE_VERSION
 *     -# Set (#SB_RET_SAME_IMAGE_VERSION & #SB_PRV_RET_LOWER_MASK) to ret
 *   -# default
 ...
 *   -# default, Set #SB_PRV_RET_UNSUPPORTED_ALGO to hash_algo
 * -# Return hash_algo
 *
 * \callgraph
 *********************************************************************************************************************/
static uint32_t cmn_drv_get_hash_algo (const uint32_t type)
{
    ...
    switch (type & SB_PRV_TLV_TYPE_CRYPTO_ALGO_MASK)
    {
        ...
        default:
        {
            hash_algo = SB_PRV_RET_UNSUPPORTED_ALGO;
            break;
        }
    }
 ...
 *   -# default, Set #SB_PRV_RET_UNSUPPORTED_ALGO to sign_algo
 * -# Return sign_algo
 *
 * \callgraph
 *********************************************************************************************************************/
static uint32_t cmn_drv_get_sign_algo (const uint32_t type)
{
    ...
    switch (type & SB_PRV_TLV_TYPE_CRYPTO_ALGO_MASK)
    {
        ...
        default:
        {
            sign_algo = SB_PRV_RET_UNSUPPORTED_ALGO;
            break;
        }
    }
 ...
 *     -# default, Set #SB_PRV_RET_UNSUPPORTED_ALGO to sign_scheme
 * -# Else
...
static uint32_t cmn_drv_get_sign_scheme (const uint32_t type, const uint32_t sign_algo)
{
        ...
        (CIP_DRV_SIGN_ALGO_RSA3072 == sign_algo) || (CIP_DRV_SIGN_ALGO_RSA4096 == sign_algo))
    {
        switch (type & SB_PRV_TLV_TYPE_CLS_SIGN_SCHEME_MASK)
        {
            ...
            default:
            {
                sign_scheme = SB_PRV_RET_UNSUPPORTED_ALGO;
                break;
            }
        }
    }
 ...
 *   -# default, Set #SB_PRV_RET_UNSUPPORTED_ALGO to cip_algo
 * -# Return cip_algo
 *
 * \callgraph
 *********************************************************************************************************************/
static uint32_t cmn_drv_get_img_cip_info_algo (const uint32_t type)
{
    ...
    switch (type & SB_PRV_TLV_TYPE_CRYPTO_ALGO_MASK)
    {
        ...
        default:
        {
            cip_algo = SB_PRV_RET_UNSUPPORTED_ALGO;
            break;
        }
    }
 ...
 *   -# default, Set #SB_PRV_RET_UNSUPPORTED_ALGO to cip_mode
 * -# Return cip_mode
 *
 * \callgraph
 *********************************************************************************************************************/
static uint32_t cmn_drv_get_img_cip_info_mode (const uint32_t type)
{
    ...
    switch (type & SB_PRV_TLV_TYPE_CLS_ICI_MODE_MASK)
    {
        ...
        default:
        {
            cip_mode = SB_PRV_RET_UNSUPPORTED_ALGO;
            break;
        }
    }
