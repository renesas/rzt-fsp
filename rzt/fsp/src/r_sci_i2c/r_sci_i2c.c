...
#define SCI_I2C_PRV_DMAC_RX_FOR_READ_TRANSFER_SETTINGS     ((TRANSFER_MODE_NORMAL <<            \
                                                             TRANSFER_SETTINGS_MODE_BITS) |     \
                                                            (TRANSFER_ADDR_MODE_FIXED <<        \
                                                             TRANSFER_SETTINGS_SRC_ADDR_BITS) | \
                                                            (TRANSFER_ADDR_MODE_INCREMENTED <<  \
                                                             TRANSFER_SETTINGS_DEST_ADDR_BITS))
#define SCI_I2C_PRV_DMAC_TX_FOR_READ_TRANSFER_SETTINGS     ((TRANSFER_MODE_NORMAL <<            \
                                                             TRANSFER_SETTINGS_MODE_BITS) |     \
                                                            (TRANSFER_ADDR_MODE_FIXED <<        \
                                                             TRANSFER_SETTINGS_SRC_ADDR_BITS) | \
                                                            (TRANSFER_ADDR_MODE_FIXED <<        \
                                                             TRANSFER_SETTINGS_DEST_ADDR_BITS))
#define SCI_I2C_PRV_DMAC_TX_FOR_WRITE_TRANSFER_SETTINGS    ((TRANSFER_MODE_NORMAL <<            \
                                                             TRANSFER_SETTINGS_MODE_BITS) |     \
                                                            (TRANSFER_ADDR_MODE_INCREMENTED <<  \
                                                             TRANSFER_SETTINGS_SRC_ADDR_BITS) | \
                                                            (TRANSFER_ADDR_MODE_FIXED <<        \
...
void             sci_i2c_rxi_dmac_isr(IRQn_Type irq);
void             sci_i2c_txi_dmac_isr(IRQn_Type irq);
 ...
 * @retval  FSP_ERR_ASSERTION           The parameter p_api_ctrl, p_dest, p_callback is NULL, bytes is 0.
 ...
 *
 * @retval  FSP_SUCCESS                 Function executed without issue.
 * @retval  FSP_ERR_ASSERTION           p_api_ctrl, p_src, p_callback is NULL.
 * @retval  FSP_ERR_NOT_OPEN            Device was not even opened.
 **********************************************************************************************************************/
...
static void sci_i2c_abort_seq_master (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    p_ctrl->p_reg->ICR |= R_SCI0_ICR_IICSCLS_Msk | R_SCI0_ICR_IICSDAS_Msk;
...
static void sci_i2c_open_hw_master (sci_i2c_instance_ctrl_t * const p_ctrl, i2c_master_cfg_t const * const p_cfg)
{
     ...
     */
    p_ctrl->p_reg->CCR1 = (uint32_t) pextend->clock_settings.snfr_value << R_SCI0_CCR1_NFCS_Pos | R_SCI0_CCR1_NFEN_Msk;

    /* Clear status flags. */
    p_ctrl->p_reg->CFCLR = (R_SCI0_CFCLR_RDRFC_Msk |
    ...
    p_ctrl->p_reg->ICFCLR = R_SCI0_ICFCLR_IICSTIFC_Msk;
...
static void sci_i2c_run_hw_master (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if (!p_ctrl->restarted)
    {
         ...
         */
        p_ctrl->p_reg->ICR = SCI_I2C_PRV_GENERATE_REQUEST(p_ctrl->p_reg->ICR, R_SCI0_ICR_IICSTAREQ_Msk);
    }
    else
    {
...
/******************************************************************************************************************//**
 * ISR for ACK/RXI interrupt called when the DMA transfer is completed
 *
 **********************************************************************************************************************/
#if SCI_I2C_CFG_DMAC_ENABLE
void sci_i2c_rxi_dmac_isr (IRQn_Type irq)
...
/******************************************************************************************************************//**
 * ISR for NACK/TXI interrupt called when the DMA transfer is completed
 *
 **********************************************************************************************************************/
#if SCI_I2C_CFG_DMAC_ENABLE
void sci_i2c_txi_dmac_isr (IRQn_Type irq)
...
static void sci_i2c_txi_handler (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if (0U == (p_ctrl->p_reg->ISR & R_SCI0_ISR_IICACKR_Msk))
    {
        ...
        if (p_ctrl->addr_total != p_ctrl->addr_loaded)
        {
            ...
            if ((p_ctrl->read) && (2U == p_ctrl->addr_loaded))
            {
                 ...
                 * - IICDL, IICINTM, IICCSC, and IICACKT settings must be preserved.
                 */
                p_ctrl->p_reg->ICR = SCI_I2C_PRV_GENERATE_REQUEST(p_ctrl->p_reg->ICR, R_SCI0_ICR_IICRSTAREQ_Msk);
            }
            ...
            else
            {
                ...
                /* Write 1byte p_ctrl->addr_low to TDR register */
                p_ctrl->p_reg->TDR = SCI_I2C_PRV_DATA_REG_MASK | p_ctrl->addr_low;
...
static void sci_i2c_tei_handler (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    else if (0U == p_ctrl->remain)
    {
        ...
        if (p_ctrl->restarted)
        {
            ...
            p_ctrl->p_reg->ICR &= ~(R_SCI0_ICR_IICSDAS_Msk | R_SCI0_ICR_IICSCLS_Msk);
        }
        else
        {
            /* Set SDA and SCL to high impedance state. */
            ...
            p_ctrl->p_reg->CCR0 &= ~(R_SCI0_CCR0_TE_Msk | R_SCI0_CCR0_RE_Msk);
...
static fsp_err_t sci_i2c_transfer_open (sci_i2c_instance_ctrl_t * const p_ctrl, i2c_master_cfg_t const * const p_cfg)
{
    ...
    if (NULL != p_cfg->p_transfer_rx)
    {
        err = sci_i2c_transfer_configure(p_ctrl, p_cfg->p_transfer_rx, SCI_I2C_DMAC_INTERRUPT_TRIGGER_RXI);
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    }

    if (NULL != p_cfg->p_transfer_tx)
    {
        err = sci_i2c_transfer_configure(p_ctrl, p_cfg->p_transfer_tx, SCI_I2C_DMAC_INTERRUPT_TRIGGER_TXI);
        if (FSP_SUCCESS != err)
        {
            if (NULL != p_cfg->p_transfer_rx)
            {
                                             ...
                                             sci_i2c_dmac_interrupt_trigger_t trigger)
{
    ...
    if (SCI_I2C_DMAC_INTERRUPT_TRIGGER_RXI == trigger)
    {
        ...
        p_info->transfer_settings_word = SCI_I2C_PRV_DMAC_RX_FOR_READ_TRANSFER_SETTINGS;
        p_info->p_src = (void *) (&(p_ctrl->p_reg->RDR));
    }
    else
    {
        ...
        p_info->transfer_settings_word = SCI_I2C_PRV_DMAC_TX_FOR_READ_TRANSFER_SETTINGS;
        p_info->p_dest                 = (void *) (&(p_ctrl->p_reg->TDR));
    }
...
static void sci_i2c_reconfigure_interrupts_for_transfer (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if (NULL != p_transfer_tx)
    {
        ...
        {
            /* Re-adjust address modes */
            p_info->transfer_settings_word = SCI_I2C_PRV_DMAC_TX_FOR_WRITE_TRANSFER_SETTINGS;
        }

        /* Set the interrupt source to RXI/TXI */
        p_ctrl->p_reg->ICR |= R_SCI0_ICR_IICACKT_Msk;
        p_ctrl->p_reg->CCR0 =
            (R_SCI0_CCR0_TE_Msk | R_SCI0_CCR0_RE_Msk | R_SCI0_CCR0_TEIE_Msk | R_SCI0_CCR0_TIE_Msk);
        FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->CCR0_b.RE, 1);
        FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->CCR0_b.TE, 1);
...
static void sci_i2c_enable_transfer_support_tx (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if ((NULL != p_transfer_tx) && (!p_ctrl->read))
    {
         ...
         */
        p_transfer_tx->p_cfg->p_info->p_src  = (void *) (p_ctrl->p_buff);
        p_transfer_tx->p_cfg->p_info->length = p_ctrl->remain;
...
static void sci_i2c_txi_send_data (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    else
    {
        ...
        if (true == p_ctrl->do_dummy_read)
        {
                ...
                (p_ctrl->total > 2U))
            {
                ...
                p_ctrl->p_cfg->p_transfer_rx->p_cfg->p_info->length = p_ctrl->total - 2U;
                ...
                p_ctrl->p_cfg->p_transfer_tx->p_cfg->p_info->length = p_ctrl->total - 2U;
     ...
     */
    p_ctrl->p_reg->TDR = SCI_I2C_PRV_DATA_REG_MASK | data;
}
...
static void sci_i2c_txi_process_nack (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if ((!p_ctrl->read) || (true == p_ctrl->do_dummy_read))
    {
         ...
         * - IICDL, IICINTM, IICCSC, and IICACKT settings must be preserved.
         */
        p_ctrl->p_reg->ICR = SCI_I2C_PRV_GENERATE_REQUEST(p_ctrl->p_reg->ICR, R_SCI0_ICR_IICSTPREQ_Msk);
    }
...
static void sci_i2c_issue_restart_or_stop (sci_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if (true == p_ctrl->restart)
    {
        ...
        p_ctrl->p_reg->ICR = (R_SCI0_ICR_IICRSTAREQ_Msk | icr);
        p_ctrl->restarted  = true;
    }
    else
    {
        /* The IICSTPREQ, IICSDAS, IICSCLS bits must be set simultaneously. */
        p_ctrl->p_reg->ICR = (R_SCI0_ICR_IICSTPREQ_Msk | icr);
    }
}
