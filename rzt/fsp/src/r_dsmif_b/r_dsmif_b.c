 ...
 * Macro definitions
 **********************************************************************************************************************/
#define DSMIF_B_VALID_UNIT_MASK      (0x3U) /* Unit0-1 */

#define DSMIF_B_CMSH_DEFAULT         (0x0U)

#define VECNUM_DSMIF0_CDRUI          ((IRQn_Type) 343)
#define VECNUM_DSMIF1_CDRUI          ((IRQn_Type) 344)

#define DSMIF_B_OPEN                 (0x44534D49U)

#define DSMIF_B_DATA_TYPE_MASK       (0x0300U)
#define DSMIF_B_DATA_CHANNEL_MASK    (0x0003U)
#define DSMIF_B_DATA_TYPE_SHIFT      (0x8U)

#define DSMIF_B_REGID_MASK           (0x000000FFU)

#define DSCESR_SUMERR_MASK           (R_DSMIF0_DSCESR_SUMERRH_Msk | R_DSMIF0_DSCESR_SUMERRL_Msk)
#define DSCESR_SCF_MASK              (R_DSMIF0_DSCESR_SCF2_Msk | R_DSMIF0_DSCESR_SCF1_Msk | R_DSMIF0_DSCESR_SCF0_Msk)

#define DSCOCESR_REG_MASK            (0x00FFFFFFU)
...
#if (1 == DSMIF_B_CFG_PARAM_CHECKING_ENABLE)

static fsp_err_t r_dsmif_b_open_param_check(dsmif_b_instance_ctrl_t * const p_instance_ctrl,
                                            adc_cfg_t const * const         p_cfg);

#endif

static void      r_dsmif_b_start(dsmif_b_instance_ctrl_t * const p_instance_ctrl);
static fsp_err_t r_dsmif_b_stop(dsmif_b_instance_ctrl_t * const p_instance_ctrl);
static void      r_dsmif_b_open_irq_cfg(dsmif_b_instance_ctrl_t * const p_instance_ctrl, adc_cfg_t const * const p_cfg);
static void      r_dsmif_b_disable_irq(IRQn_Type irq);

void dsmif_b_cdrui_isr(void);
...
const adc_api_t g_adc_on_dsmif_b =
{
    .open          = R_DSMIF_B_Open,
    .scanStart     = R_DSMIF_B_ScanStart,
    .scanStop      = R_DSMIF_B_ScanStop,
    .scanStatusGet = R_DSMIF_B_StatusGet,
    .read32        = R_DSMIF_B_Read,
    .close         = R_DSMIF_B_Close,
    .callbackSet   = R_DSMIF_B_CallbackSet
};
...
{
    dsmif_b_instance_ctrl_t * p_instance_ctrl = (dsmif_b_instance_ctrl_t *) p_ctrl;
    fsp_err_t                 err             = FSP_SUCCESS;

#if (1 == DSMIF_B_CFG_PARAM_CHECKING_ENABLE)
    err = r_dsmif_b_open_param_check(p_instance_ctrl, p_cfg);
    ...
    r_dsmif_b_open_irq_cfg(p_instance_ctrl, p_cfg);

    dsmif_b_extended_cfg_t const * p_cfg_extend = p_cfg->p_extend;
    ...
    for (uint32_t ch = 0U; ch < DSMIF_B_MAX_NUM_CHANNELS; ch++)
    {
        ...
        if (p_cfg_extend->p_channel_cfgs[ch] != NULL)
        {
            ...
            uint32_t ckdiv;
            if (DSMIF_B_CLOCK_CTRL_SLAVE == p_cfg_extend->p_channel_cfgs[ch]->ckdir)
            {
                ckdiv = 0U;
            }
            else
    ...
    p_instance_ctrl->opened = DSMIF_B_OPEN;
...
fsp_err_t R_DSMIF_B_ScanStart (adc_ctrl_t * p_ctrl)
{
    ...
    FSP_ERROR_RETURN(DSMIF_B_OPEN == p_instance_ctrl->opened, FSP_ERR_NOT_OPEN);
#endif

    r_dsmif_b_start(p_instance_ctrl);
...
fsp_err_t R_DSMIF_B_ScanStop (adc_ctrl_t * p_ctrl)
{
    ...
    err = r_dsmif_b_stop(p_instance_ctrl);
...
fsp_err_t R_DSMIF_B_CfgSet (adc_ctrl_t * p_ctrl, adc_cfg_t const * const p_cfg)
{
    ...
    dsmif_b_extended_cfg_t const * p_cfg_extend = (dsmif_b_extended_cfg_t *) p_cfg->p_extend;
...
fsp_err_t R_DSMIF_B_StatusGet (adc_ctrl_t * p_ctrl, adc_status_t * p_status)
{
    ...
    /* DSCOCESR register (0-23bit), DSCESR register (24-31bit) */
    uint32_t err_sts_val;

    /* Place SUMERRH(17bit), SUMERRL(16bit) in 29-28bit  */
    err_sts_val = ((p_instance_ctrl->p_reg->DSCESR & DSCESR_SUMERR_MASK) << 12U);

    /* 2-0bit "SCF2-SCF0" ->Place SCF2-SCF0(2-0bit) in 26-24bit  */
    err_sts_val           |= ((p_instance_ctrl->p_reg->DSCESR & DSCESR_SCF_MASK) << 24U);
    err_sts_val           |= (p_instance_ctrl->p_reg->DSCOCESR & DSCOCESR_REG_MASK);
    p_status->error_status = err_sts_val;

    return FSP_SUCCESS;
}

/*******************************************************************************************************************//**
 * Returns the DSMIF Status Register.
 *
 * @retval  FSP_SUCCESS                No software scan is in progress.
 * @retval  FSP_ERR_ASSERTION          An input pointer was NULL.
 * @retval  FSP_ERR_NOT_OPEN           Instance control block is not open.
...
fsp_err_t R_DSMIF_B_RegStatusGet (adc_ctrl_t * p_ctrl, uint32_t * p_status)
{
    ...
    *p_status = p_instance_ctrl->p_reg->DSCOCNSR; /* Channel Overcurrent Notification Status Register */
...
fsp_err_t R_DSMIF_B_Read (adc_ctrl_t * p_ctrl, adc_channel_t const reg_id, uint32_t * const p_data)
{
    ...
    FSP_ERROR_RETURN(DSMIF_B_OPEN == p_instance_ctrl->opened, FSP_ERR_NOT_OPEN);
    FSP_ASSERT(0U != ((1U << ((uint32_t) reg_id & DSMIF_B_REGID_MASK)) & p_instance_ctrl->channel_mask));
#endif

    adc_channel_t          channel         = (adc_channel_t) (reg_id & DSMIF_B_DATA_CHANNEL_MASK);
    dsmif_b_current_data_t dsmif_data_type =
        (dsmif_b_current_data_t) ((reg_id & DSMIF_B_DATA_TYPE_MASK) >> DSMIF_B_DATA_TYPE_SHIFT);
    if (DSMIF_B_CURRENT_DATA == dsmif_data_type)
    {
        p_instance_ctrl->results.results_32[channel] = p_instance_ctrl->p_reg->CH[channel].DSCDR;
    }
    else if (DSMIF_B_CURRENT_DATA_CAPTURE_A == dsmif_data_type)
    {
...
fsp_err_t R_DSMIF_B_Close (adc_ctrl_t * p_ctrl)
{
    ...
    /* Disable interrupts. */
    r_dsmif_b_disable_irq(p_instance_ctrl->p_cfg->scan_end_irq);
    ...
    (void) r_dsmif_b_stop(p_instance_ctrl);
...
{
    dsmif_b_instance_ctrl_t * p_ctrl = (dsmif_b_instance_ctrl_t *) p_api_ctrl;
                                             ...
                                             adc_cfg_t const * const         p_cfg)
{
    ...
    FSP_ERROR_RETURN(DSMIF_B_OPEN != p_instance_ctrl->opened, FSP_ERR_ALREADY_OPEN);

    /* Verify the unit exists on the MCU. */
    FSP_ERROR_RETURN(((1U << p_cfg->unit) & DSMIF_B_VALID_UNIT_MASK), FSP_ERR_IP_UNIT_NOT_PRESENT);
...
{
    fsp_err_t err     = FSP_SUCCESS;
    uint32_t  timeout = DSMIF_B_STOP_TIMEOUT;
...
void dsmif_b_cdrui_isr (void)
{
    ...
    dsmif_b_instance_ctrl_t * p_instance_ctrl = (dsmif_b_instance_ctrl_t *) R_FSP_IsrContextGet(irq);
